...
def safe_handler(func):
    async def wrapper(*args, **kwargs):
        try:
            ...
            await bot.send_message(7524032836, error_text)
    return wrapper
...
BOT_NAME = os.environ.get("BOT_NAME", "teamraj music")
BOT_LINK = os.environ.get("BOT_LINK", "https://t.me/Teamrajmusicbot")
...
async def start_handler(_, message):
    ...
    buttons = [
        [
            ...
            InlineKeyboardButton(f"📢 {updates_text}", url="https://t.me/bestshayri_alone")
        ],
        [
            InlineKeyboardButton(f"💬 {support_text}", url="https://t.me/Alone_ki_duniya_hai"),
...
async def process_play_command(message: Message, query: str):
    ...
    if isinstance(result, dict) and "playlist" in result:
        ...
        if secs > MAX_DURATION_SECONDS:
            await processing_message.edit(
                "❌ Streams longer than 15 min are not allowed. If u are the owner of this bot contact @bestshayri_raj to upgrade your plan"
            )
            return
...
async def fallback_local_playback(chat_id: int, message: Message, song_info: dict):
    ...
    try:
        ...
        base_caption = (
            "<blockquote>"
            "<b>🎧  @teamrajweb Streaming</b> (Local Playback)\n\n"
